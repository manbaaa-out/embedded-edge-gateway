...
单进程串起「串口采集 → 云端上行」与「云端下发 → 串口控制」的**双向**数据链路:
STM32 节点经 UART 上报传感器数据,网关解析后双写 SQLite + MQTT,并通过内嵌的 epoll HTTP 服务提供实时网页与历史曲线;运维可经 MQTT 下发命令,网关组帧、写串口、等 ACK、超时重发。
...
- [运行与测试](#运行与测试)
  ...
  - [方式二:真实 STM32(真机部署)](#方式二真实-stm32真机部署)
  - [验证上行数据](#验证上行数据)
  - [下行命令](#下行命令)
- [配置](#配置)
- [部署](#部署)
...
- **双向链路**:上行(采集→双写→上云)+ 下行(MQTT 命令→串口控制),命令带 seq、ACK 配对、超时重发。
- **统一事件驱动**:串口数据、下行命令(eventfd)、超时重试(timerfd)、SIGHUP 信号(signalfd)全封装成 channel,挂在主线程同一个 epoll 循环上。
- **单一写者**:串口 fd 只由主线程写,跨线程命令经线程安全队列 + eventfd 投递,`SerialPort` 无需加锁。
- **双写**:每条记录由线程池 worker 同时落本地 SQLite 与 MQTT 上行,互不阻塞。
- **读写并发**:HTTP 查询用独立的只读 SQLite 连接(`SQLITE_OPEN_READONLY`),靠 WAL 与主链写连接并发,查询不阻塞落库。
...
- **可靠工程化**:RAII 管控资源与析构顺序(无 use-after-free)、致命错误优雅退出交由 systemd 重启接管、编译零警告(`-Wall -Wextra -Wpedantic`)。
...
**监控**:HTTP 监控服务跑在独立线程,持有一个**只读** SQLite 连接,靠 WAL 与主链的读写连接并发——查询不阻塞落库。浏览器访问即可看到实时设备卡片与 uPlot 历史曲线。

**STM32 节点内部**(图左):三任务按 命令(P3)> 发送(P2)> 采样上报(P1)分工,经 `xTxQueue` / `g_rx_stream` 流缓冲 / `g_sensor_mutex` 解耦;USART1 `ReceiveToIdle + DMA` 收、RX-IDLE 中断 `FromISR` 唤醒命令任务;三任务在事件组 `g_wdg_events` 各自「打卡」,集齐才喂 IWDG(2s),配合 tickless Sleep 低功耗。节点工程见 [N6_freertos](https://github.com/manbaaa-out/stm32-learning/tree/main/N6_freertos)。
...
# 运行 / 测试用:MQTT broker、MQTT 客户端、SQLite CLI、虚拟串口
...
产物为单个可执行 `build/gateway`,命令行参数是**配置文件路径**(缺省 `/etc/gateway.conf`)。

## 运行与测试

网关启动即连接 MQTT broker(连不上会致命退出),所以两种方式都先确保 broker 在跑:
...
**② 准备一份指向虚拟串口的配置**(仓库默认 `serial_path` 是真机串口,这里复制一份改掉):
...
在树莓派上用真实 STM32 节点经 UART 通信。STM32 端固件需按 [M5 帧协议](docs/m5_frame_protocol.md) 组帧发送,并能响应下行命令(`0x20/0x21/0x22`)回 ACK(`0x06`)/ 查询应答(`0x05`)。
...
- **树莓派板载 GPIO UART**:STM32 `TX → RPi RXD(GPIO15 / pin10)`、`RX → RPi TXD(GPIO14 / pin8)`、`GND ↔ GND`;设备名是 `/dev/serial0`(通常软链到 `/dev/ttyAMA0` 或 `/dev/ttyS0`)。

**② 树莓派板载 UART 额外配置**(用 USB-TTL 可跳过):

```bash
sudo raspi-config      # Interface Options → Serial Port:
                       #   "login shell over serial" 选 No(关掉串口控制台)
                       #   "serial port hardware"    选 Yes(打开串口硬件)
```

确认 `/boot/firmware/config.txt`(旧系统为 `/boot/config.txt`)含 `enable_uart=1`;Pi 3/4 建议加 `dtoverlay=disable-bt`,把稳定的 PL011 UART 让给 `/dev/ttyAMA0`。改完重启。

**③ 串口权限**(免 sudo 读写串口),加入 `dialout` 组后重新登录:
...
**④ 配置串口设备与波特率**(波特率必须与 STM32 固件一致,本项目默认 115200)。编辑 `src/deploy/gateway.conf`:

```ini
serial_path = /dev/ttyUSB0     # 或 /dev/serial0
serial_baud = 115200
```
...
```bash
./build/gateway src/deploy/gateway.conf
```

真机长期运行建议用 systemd 托管,见 [部署](#部署)。验证方式与虚拟串口一致。
...
热加载:改完配置发 `SIGHUP`(`kill -HUP $(pgrep -x gateway)`,或 systemd 下 `systemctl reload gateway`),无需重启。A 档改内存即生效;B 档按 diff 仅重建变化的资源;C 档热加载会忽略并告警。
...
sudo install -m 644 src/deploy/gateway.conf /etc/gateway.conf      # 按需修改 serial_path/db_path 等
