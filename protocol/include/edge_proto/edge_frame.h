 ...
 *
 * 由网关端 src/protocol/FrameParser.cpp(C++ 类 + std::vector + std::function)
 * 与节点端 Protocol/frame_parser.c(struct + 定长数组 + 函数指针)归一而来。
 * 取节点侧的 C 结构为基准:定长数组无堆分配,MCU 与 Linux 都能用;
 * 网关侧在其上包一层薄 C++ RAII 即可拿回 vector/std::function 的手感。
 *
 * 【本层不做 I/O】原网关 FSM 直接往 std::cerr 打日志 —— 协议层不该决定
 * 错误怎么呈现,更不该在 MCU 上拖进 stdio。改为累计到 edge_parser_stats_t,
 * 网关拿去走 LOG_WARN,节点可以塞进 0x04 状态帧上报。
 * ------------------------------------------------------------------------- */
 ...
 *
 * 契约:out 容量必须 >= EDGE_FRAME_MAX(调用方用定长数组 + 静态断言钉死即可,
 *       不为此加一个运行期参数去比较两个编译期已知的数)。
 *       payload 可为 NULL,当且仅当 payload_len == 0。
 *       payload_len > EDGE_PAYLOAD_MAX 视为参数非法,返回 0。
 *
...
typedef struct {
    ...
    uint32_t len_err;   /* LEN 越界(§5.3 Fail Fast,把灾难压缩到只丢 1 帧) */
    uint32_t crc_err;   /* CRC 不匹配 */
    uint32_t resync;    /* 异常路径回到起点的次数(含 DELIVER 态收到意外字节) */
} edge_parser_stats_t;

/* 收齐一帧且 CRC 校验通过时回调。
 * user 透传调用方上下文 —— 节点侧传 NULL 即可;网关侧靠它把 C 回调
 * 弹回 C++ 对象(std::function 成员),避免在协议层出现全局变量。 */
...
/* 置初态 + 装回调。cb 可为 NULL(只统计不交付,如做链路探测)。 */
