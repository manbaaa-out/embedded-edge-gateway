// 装配层:持有各层资源、把事件源挂上 epoll、跑主循环。不含业务逻辑本身。
//
// 两条贯穿全文的纪律:
//   一、串口只有一个写者(主线程)。别处产生的命令一律经队列 + eventfd 投递过来,
//       省掉的不只是一把锁,还有「两条线程交错写把帧撕开」这类极难复现的故障。
//   二、错误分三级 —— 致命(启动期抛,交 systemd 重启)、降级(丢一个功能保采集)、
//       可恢复(重试)。热加载失败属第二级:失败只丢这一项变更,进程照常跑。
//
// 成员声明顺序即析构逆序,改动前先看类头那张表。
